...
describe('SplitWiseGroup contract', function () {
  ...
  describe('Joining the group', function () {
    ...
    it('Should not allow members to join if already a member', async function () {
      await expect(
        splitWiseGroup
          .connect(member1)
          .join({ value: ethers.utils.parseEther('0.1') })
  ...
  describe('Add expenses to the group', function () {
    it('should allow the owner to add an expense', async function () {
      ...
      expect(
        debt1to2,
        'Debt from member1 to member2 should be 0.5 HBAR'
      ...
      expect(
        debt3to1,
        'Debt from member3 to member1 should be 0.5 HBAR'
      ).to.equal(ethers.utils.parseEther('0.5'))
      expect(
        debt3to2,
        'Debt from member3 to member2 should be 1.0 HBAR'
  ...
  describe('Transfer from group', function () {
    it('should transfer funds from the contract to the recipient', async function () {
      ...
      const amount = '0.25'
      const amountToSend = ethers.utils.parseUnits(amount, 8)
      ...
      expect(recipientBalanceAfter).to.equal(
        initialBalance.add(ethers.utils.parseEther(amount))
      ) // Recipient's balance should have increased by the transferred amount
    })
